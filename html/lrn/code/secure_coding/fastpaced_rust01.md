...
Names that *start* with an underscore are regular names, it\'s just that
...
Of course, when destructuring a tuple, `_` can be used to throw away
part of it:
...
And here\'s a function that returns a 32-bit signed integer. The arrow
indicates its return type:
...
And that\'s why \"omitting the semicolon at the end of a function\" is
the same as returning, ie. these are equivalent:
...
In this example, `std` is a *crate* (\~ a library), `cmp` is a *module*
(\~ a source file), and `min` is a *function*:
...
`use` directives can be used to \"bring in scope\" names from other
namespace:
...
Within `use` directives, curly brackets have another meaning: they\'re
\"globs\". If we want to import both `min` and `max`, we can do any of
these:
...
Types are namespaces too, and methods can be called as regular
functions:
...
`str` is a primitive type, but many non-primitive types are also in
scope by default.
...
(Which in turns re-exports a lot of symbols, like `Vec`, `String`,
`Option` and `Result`).
...
This is called \"struct update syntax\", can only happen in last
position, and cannot be followed by a comma.
...
Variable bindings are immutable by default, which means their interior
can\'t be mutated:
...
An `impl` block is always *for* a type, so, inside that block, `Self`
means that type:
...
Some traits are *markers* - they don\'t say that a type implements some
methods, they say that certain things can be done with a type.

For example, `i32` implements trait `Copy` (in short, `i32` *is*
`Copy`), so this works:
...
It also works if a function takes a *mutable* reference - but only if
our variable binding is also `mut`.
...
Some traits are so common, they can be implemented automatically by
...
Type parameters usually have *constraints*, so you can actually do
something with them.
...
Constraints can be more complicated: they can require a type parameter
...
This is lovingly called [turbofish syntax](https://turbo.fish/), because
`::<>` looks like a fish.
...
Speaking of `Vec`, it comes with a macro that gives more or less \"vec
literals\":
...
All of `name!()`, `name![]` or `name!{}` invoke a macro. Macros just
expand to regular code.
...
message, and the file name / line number of the error, if enabled:
...
Some methods also panic. For example, the `Option` type can contain
something, or it can contain nothing. If `.unwrap()` is called on it,
and it contains nothing, it panics:
...
The lifetime of a reference cannot exceed the lifetime of the variable
binding it borrows:
...
This allows returning references whose lifetime depend on the lifetime
of the arguments:
...
Structs can also be *generic over lifetimes*, which allows them to hold
references:
...
There is a special lifetime, named `'static`, which is valid for the
...
`String`. It\'s been allocated dynamically, and it will be freed. Its
lifetime is *less* than the whole program (even though it happens to be
in `main`).
...
Speaking of: in a struct literal, when a field is set to a variable
...
The `..` syntax is just range literals. Ranges are just a few structs
defined in the standard library.

They can be open-ended, and their rightmost bound can be inclusive, if
it\'s preceded by `=`.
...
The `*` operator can be used to *dereference*, but you don\'t need to do
that to access fields or call methods:
...
Closures are just functions of type `Fn`, `FnMut` or `FnOnce` with some
captured context.

Their parameters are a comma-separated list of names within a pair of
pipes ( `|`). They don\'t *need* curly braces, unless you want to have
multiple statements.
...
An `FnMut` needs to be mutably borrowed to be called, so it can only be
...
`FnOnce` closures can only be called once. They exist because some
closure move out variables that have been moved when captured:
...
This is enforced naturally, as `FnOnce` closures need to be *moved* in
order to be called.
...
And, if you need convincing that our closure *does* move `s`, this is
illegal too:
...
You can even move a reference to some of a function\'s arguments, into a
closure it returns:
...
Writing Rust is a very different experience from reading Rust. On one
hand, you\'re not reading the *solution* to a problem, you\'re actually
solving it. On the other hand, the Rust compiler helps out a *lot*.

For all of the intentional mistakes made above (\"this code is
illegal\", etc.), rustc always has very good error messages *and*
insightful suggestions.

And when there\'s a hint missing, the compiler team is [not afraid to
add
it](https://mobile.twitter.com/fasterthanlime/status/1219601989404954624).

For more Rust material, you may want to check out:

I also [blog about Rust](/tags/rust/) and [tweet about
Rust](https://twitter.com/fasterthanlime) a lot, so if you liked this
